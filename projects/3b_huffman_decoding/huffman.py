...
def huffman_decode(encoded_file, decoded_file):
    ...
    try:
        ...
        with open(encoded_file) as file_object:
            line_list = file_object.readlines()
            for line in line_list:
                if freq_not_read:
                    # Call Function
                    freq_list = parse_header( line )
                    freq_not_read = False
                # Read huffman code
                elif line != None:
                    huff_code = list(line)
...
def huff_decode_helper( node, code ):
    """
    signature:
    ...
    traverse = code.pop(0)
    if traverse == '0':
        char = huff_decode_helper( node.left, code )
    elif traverse == '1':
        char = huff_decode_helper( node.right, code )
    # print(char)
    return char
...
def parse_header(header_string):
    ...
    """
    chars = iter(header_string.split())
    freq_list = [0]*256
    for char in chars:
        # read freq line two values at a time, using next
        freq_list[int(char)] = int(next(chars))
    return freq_list
