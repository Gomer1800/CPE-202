...
Python implementation of the Binary Heap ADT, using the MaxHeap approach. Largest key value is always at front.
This heap contains integers that represent both the priority and name of the object

The Bubbling down step of the Bottom Up approach consists of applying a percdown, starting at the last parent through the first
"""
class MaxHeap:
    ...
    def dequeue(self):
        ...
        -dequeue top of heap tree (this is the max val)
        -insert last child into top of heap tree
        -PercDown the top of tree
        """
        if self.is_empty(): return None
        max, self.heap_list[1] = self.heap_list[1], self.heap_list.pop(self.num_items)
        self.num_items-=1
        #print("deq num items :", self.num_items)
        if self.num_items > 0: self.perc_down(1)
        return max
    ...
    def build_heap(self, alist):
        ...
        # Starting at final parent through first parent, PercDown
        while i >= 1:
            self.perc_down(i)
            i-=1
    ...
    def perc_down(self, i):
        ...
        if i == self.num_items: return               # edge case, index is last item
        left = 0
        right = 0
    ...
    def heap_sort_ascending(self, alist):
        ...
        size = len(alist)
        self.build_heap(alist[0:len(alist)])
        #print("alist",alist)
        for i in range(size):
            removed = self.dequeue()
            alist[-1-i] = removed
        #print("sorted",self.heap_list[-1-size: -1])
